...
std::string XmlWriter::xml()
{
	return _xml;
}

XmlWriter& XmlWriter::clear()
{
	_xml = _tag = "";
	position = 0;
	return *this;
...
XmlWriter& XmlWriter::indent(bool ind)
{
	_indented = ind;
	return *this;
}
...
void main()
{
	...
	std::string temp = bcwtr1.element("child1", "child1's body").element("child2", "child2's body").xml();
	std::cout << "\n  " << bcwtr1.xml();
	bcwtr1.clear();
	std::cout << "\n  " <<
		cwtr.start("parent")
