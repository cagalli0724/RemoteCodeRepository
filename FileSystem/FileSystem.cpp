...
bool Directory::createAll(const std::string& path) {
	std::vector<std::string> partten;
	std::string tmp = path.substr(0,path.find_last_of("\\"));
	std::string tmp2 = path.substr(path.find_last_of("\\") + 1, path.length());
	partten.push_back(tmp2);
	while (!exists(tmp))
	{
		tmp2 = tmp.substr(tmp.find_last_of("\\") + 1, tmp.length());
		tmp = tmp.substr(0, tmp.find_last_of("\\"));
		partten.push_back(tmp2);
	}
	for (int i = partten.size() - 1; i >= 0; i--) {
		tmp = tmp + "\\" + partten[i];
...
{
	FileSystem::Directory::createAll("C:\\vs\\project4-1\\Test1\\Test2\\Test3");
